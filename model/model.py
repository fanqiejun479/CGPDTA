...
class Model(pl.LightningModule):
    ...
    def forward(self, input_id, attention_mask, x, edge_index, edge_attr, batch, prot_data, pock_data):
        # print(input_id.device, attention_mask.device, x.device, edge_index.device, batch.device, prot_data.device,
        #       pock_data.device)
    ...
    def test_epoch_end(self, outputs):
        ...
        print(f'MSE_score : {mse}')
        print(f'RMSE_score : {rmse}')
        print(f'CI_score : {CI}')
        print(f'r2_score : {r2}')
        print(f'rm2_score : {rm2}')
        print(f'pearson_score : {pearson}')
        print(f'spearman_score : {spearman}')

        self.log("test_mse", mse, on_step=False, on_epoch=True, logger=True)
        self.log("test_rmse", rmse, on_step=False, on_epoch=True, logger=True)
        self.log("test_CI", CI, on_step=False, on_epoch=True, logger=True)
        self.log("test_r2", r2, on_step=False, on_epoch=True, logger=True)
        self.log("test_rm2", rm2, on_step=False, on_epoch=True, logger=True)
        self.log("test_pearson", pearson, on_step=False, on_epoch=True, logger=True)
        self.log("test_spearman", spearman, on_step=False, on_epoch=True, logger=True)
