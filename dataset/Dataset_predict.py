...
sys.path.append(r'/data/qfan/qfan-test')
...
class Dataset(InMemoryDataset):
    def __init__(self, root=None, dataset=None, data=None, smile_graph=None, clique_type=None, split_type=None,
                 d_tokenizer=None,
        ...
        if 'test' in self.dataset:
            self.process(data, smile_graph, drug_maxLength, prot_maxLength, pock_maxLength, clique_type,self.processed_paths[1])
    ...
    def processed_file_names(self):
        return [self.dataset + self.clique_type + self.split_type + '.pt',self.dataset + self.clique_type + self.split_type + '.pt']
    ...
    def process(self, data, smile_graph, drug_maxLength, prot_maxLength, pock_maxLength, clique_type,path):
        ...
        y = data['Label']
        data_list = []
        for i in tqdm(range(len(data)), desc='Generating Graph file...', total=len(data)):
            smiles = xd[i]
            prot = xt[i]
            pocket = xp[i]
            pdb = pdb_list[i]
...
class DataModule(pl.LightningDataModule):
    ...
    def prepare_data(self):
        ...
        self.df = pd.read_csv(self.dataFolder + '/test.csv')
        compound_iso_smiles = list(self.df['compound_iso_smiles'])
        pdb = list(self.df['target_name'])

        processed_path = 'data/' + self.task_name + '/process/processed/' + self.task_name + '_test_'+ self.clique_type + self.split_type + '.pt'
            ...
            if os.path.isfile(graph_path):
                ...
                for smile in tqdm(list(compound_iso_smiles), desc='generating graph dict...',
                    ...
                    if self.clique_type == 'None':
                        ...
                        g = smile_to_clique_graph_p(smile, pdb, self.clique_type)
                        if g == None:
                            continue
                        else:
                            smile_graph[smile] = g
    ...
    def setup(self, stage=None):
        ...
        if self.load_testData is True:
            self.test_dataset = Dataset(self.dataFolder, str(self.task_name) + '_test_', self.df,
                                        self.smile_graph,
